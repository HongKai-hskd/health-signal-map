...
async function api<T>(url: string, init?: RequestInit) {
  ...
  try {
    ...
    throw new Error(response.ok ? "服务器返回了无法解析的响应。" : "服务暂时不可用，请稍后再试。");
  }
  if (!response.ok) throw new Error(payload.error ?? "发生了一点问题。");
  return payload as T;
}

function ChoiceCard({ label, description, icon: Icon, selected, disabled, onClick }: ChoiceProps) {
  return (
    <button type="button" className={`choice-card ${selected ? "choice-card-selected" : ""}`} onClick={onClick} aria-pressed={selected} disabled={disabled}>
...
function Field({ label, value, onChange, min, max, suffix }: { label: string; value: number | undefined; onChange: (value: number | undefined) => void; min: number; max: number; suffix: string }) {
  return <label className="field-shell"><span className="field-label">{label}</span><span className="field-control"><input type="number" inputMode="decimal" step={suffix === "岁" ? 1 : 0.1} min={min} max={max} value={value ?? ""} onChange={(event) => onChange(event.target.value ? Number(event.target.value) : undefined)} required /><span>{suffix}</span></span></label>;
}
...
export default function Home() {
  ...
  if (stepIndex === 0) content = choiceStep(<><ChoiceCard label="女性" icon={UserRound} selected={form.gender === "woman"} disabled={saving} onClick={() => void choose("identity", { gender: "woman" })} /><ChoiceCard label="男性" icon={UserRound} selected={form.gender === "man"} disabled={saving} onClick={() => void choose("identity", { gender: "man" })} /><ChoiceCard label="非二元" icon={UsersRound} selected={form.gender === "nonbinary"} disabled={saving} onClick={() => void choose("identity", { gender: "nonbinary" })} /><ChoiceCard label="不方便透露" icon={Moon} selected={form.gender === "prefer_not_to_say"} disabled={saving} onClick={() => void choose("identity", { gender: "prefer_not_to_say" })} /></>, "01 / 你的基础", "先从你自己开始。", "几个真实信号就够了，你的回答会决定接下来的节奏和适合你的支持方式。");
  else if (stepIndex === 1) content = choiceStep(<><ChoiceCard label="感觉更轻盈" description="让日常身体状态更松弛" icon={Scale} selected={form.goal === "feel_lighter"} disabled={saving} onClick={() => void choose("goal", { goal: "feel_lighter" })} /><ChoiceCard label="变得更强壮" description="建立能力，而不只是追逐数字" icon={Dumbbell} selected={form.goal === "get_stronger"} disabled={saving} onClick={() => void choose("goal", { goal: "get_stronger" })} /><ChoiceCard label="建立稳定习惯" description="找到可以不断重复的节奏" icon={RefreshCw} selected={form.goal === "build_consistency"} disabled={saving} onClick={() => void choose("goal", { goal: "build_consistency" })} /></>, "02 / 你的目标", "什么会让你觉得真的有收获？", "这里没有标准答案，重要的是选一个在忙碌的周二也依然值得坚持的方向。");
  else if (stepIndex === 2) content = choiceStep(<><ChoiceCard label="我正要重新开始" description="每周运动 0–1 天" icon={Sparkles} selected={form.activityLevel === "new"} disabled={saving} onClick={() => void choose("activity", { activityLevel: "new", exerciseDays: 1 })} /><ChoiceCard label="我有自己的节奏" description="每周运动 2–3 天" icon={Activity} selected={form.activityLevel === "steady"} disabled={saving} onClick={() => void choose("activity", { activityLevel: "steady", exerciseDays: 3 })} /><ChoiceCard label="我经常运动" description="每周运动 4 天以上" icon={Zap} selected={form.activityLevel === "frequent"} disabled={saving} onClick={() => void choose("activity", { activityLevel: "frequent", exerciseDays: 5 })} /></>, "03 / 你的节奏", "现在的你，通常多久动一次？", "我们会把你当下的节奏作为起点，而不是拿它来给你打分。");
