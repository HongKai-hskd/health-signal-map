...
function ReportExtras({ result, exporting, onExport }: { result: PublicHealthResult; exporting: boolean; onExport: () => void }) {
  ...
  return <section className="results-extras" aria-label="报告工具与行动计划"><div className="extras-heading"><div><p className="eyebrow"><Sparkles size={14} /> 下一步怎么做</p><h2>{actionPlan.length ? "把报告变成这周能执行的动作。" : "先把这份初步报告带走。"}</h2></div><button type="button" className="export-report-button" onClick={onExport} disabled={exporting}><ArrowRight size={15} /> {exporting ? "准备中…" : "导出报告"}</button></div>{actionPlan.length ? <div className="action-plan-grid">{actionPlan.map((item) => <article className="action-plan-card" key={item.week}><div className="action-plan-meta"><span>第 {item.week} 周</span><span>{item.focus}</span></div><h3>{item.title}</h3><p>{item.description}</p></article>)}</div> : <p className="extras-copy">报告会以 JSON 文件下载到你的设备，方便你自己保存；完整解锁后还会包含分阶段的行动计划。</p>}</section>;
}
