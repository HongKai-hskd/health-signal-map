...
创建一笔 `wechat_mock` 待支付订单，不会立即解锁订阅。服务端返回一个携带短时 `checkoutToken` 的扫码地址；该 token 是模拟付款方进入收银台的能力凭证，不会在桌面端订单状态接口中返回。未过期的重复创建请求会复用同一笔 pending 订单。
...
成功后先以条件更新将订单从 `pending` 改为 `paid`，再激活该订单对应 session 的订阅；过期订单不会被确认，重复确认保持第一次的 `paidAt`，不会重复扣款或创建第二个订阅。二维码过期返回 `409`，未知 token 返回 `404`。
